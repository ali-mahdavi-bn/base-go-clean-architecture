...
// Load account permissions to casbin
type Casbinx struct {
...
type policyQueueItem struct {
	RoleID    string
	Resources entities.MenuResources
}
...
func (a *Casbinx) load(ctx context.Context) error {
	start := time.Now()
	roleResult, err := a.RoleDAL.Query(ctx, entities.RoleQueryParam{
		Status: entities.RoleStatusEnabled,
	}, entities.RoleQueryOptions{
...
func (a *Casbinx) queryRoleResources(ctx context.Context, roleID string) (entities.MenuResources, error) {
	menuResult, err := a.MenuDAL.Query(ctx, entities.MenuQueryParam{
		RoleID: roleID,
	...
	menuResourceResult, err := a.MenuResourceDAL.Query(ctx, entities.MenuResourceQueryParam{
		MenuIDs: menuIDs,
	})
	if err != nil {
		return nil, err

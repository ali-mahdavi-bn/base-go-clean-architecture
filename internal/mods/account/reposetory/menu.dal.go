...
func GetMenuDB(ctx context.Context, defDB *gorm.DB) *gorm.DB {
	return util.GetDB(ctx, defDB).Model(new(entities.Menu))
}
...
func (a *Menu) Query(ctx context.Context, params entities.MenuQueryParam, opts ...entities.MenuQueryOptions) (*entities.MenuQueryResult, error) {
	var opt entities.MenuQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
	...
	if v := params.Public; v {
		db = db.Where("public = ?", v)
	}
	...
	var list entities.Menus
	pageResult, err := util.WrapPageQuery(ctx, db, params.PaginationParam, opt.QueryOptions, &list)
	if err != nil {
		return nil, errors.WithStack(err)
	}

	queryResult := &entities.MenuQueryResult{
...
func (a *Menu) Get(ctx context.Context, id string, opts ...entities.MenuQueryOptions) (*entities.Menu, error) {
	var opt entities.MenuQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
...
func (a *Menu) GetByCodeAndParentID(ctx context.Context, code, parentID string, opts ...entities.MenuQueryOptions) (*entities.Menu, error) {
	var opt entities.MenuQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
...
func (a *Menu) GetByNameAndParentID(ctx context.Context, name, parentID string, opts ...entities.MenuQueryOptions) (*entities.Menu, error) {
	var opt entities.MenuQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
...
// Create a new menu.
func (a *Menu) Create(ctx context.Context, item *entities.Menu) error {
	result := GetMenuDB(ctx, a.DB).Create(item)
	return errors.WithStack(result.Error)
}

// Update the specified menu in the database.
func (a *Menu) Update(ctx context.Context, item *entities.Menu) error {
...
func (a *Menu) Delete(ctx context.Context, id string) error {
	result := GetMenuDB(ctx, a.DB).Where("id=?", id).Delete(new(entities.Menu))
	return errors.WithStack(result.Error)
}

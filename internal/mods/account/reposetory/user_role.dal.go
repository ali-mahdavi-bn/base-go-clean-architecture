...
func GetUserRoleDB(ctx context.Context, defDB *gorm.DB) *gorm.DB {
	return util.GetDB(ctx, defDB).Model(new(entities.UserRole))
}

// User roles for AccountModules
type UserRole struct {
	DB *gorm.DB
}

// Query user roles from the database based on the provided parameters and options.
func (a *UserRole) Query(ctx context.Context, params entities.UserRoleQueryParam, opts ...entities.UserRoleQueryOptions) (*entities.UserRoleQueryResult, error) {
	var opt entities.UserRoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}

	db := a.DB.Table(fmt.Sprintf("%s AS a", new(entities.UserRole).TableName()))
	if opt.JoinRole {
		db = db.Joins(fmt.Sprintf("left join %s b on a.role_id=b.id", new(entities.Role).TableName()))
	...
	var list entities.UserRoles
	pageResult, err := util.WrapPageQuery(ctx, db, params.PaginationParam, opt.QueryOptions, &list)
	if err != nil {
		return nil, errors.WithStack(err)
	}

	queryResult := &entities.UserRoleQueryResult{
...
func (a *UserRole) Get(ctx context.Context, id string, opts ...entities.UserRoleQueryOptions) (*entities.UserRole, error) {
	var opt entities.UserRoleQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}

	item := new(entities.UserRole)
...
func (a *UserRole) Create(ctx context.Context, item *entities.UserRole) error {
	result := GetUserRoleDB(ctx, a.DB).Create(item)
	return errors.WithStack(result.Error)
}

// Update the specified user role in the database.
func (a *UserRole) Update(ctx context.Context, item *entities.UserRole) error {
...
func (a *UserRole) Delete(ctx context.Context, id string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("id=?", id).Delete(new(entities.UserRole))
	return errors.WithStack(result.Error)
}

func (a *UserRole) DeleteByUserID(ctx context.Context, userID string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("user_id=?", userID).Delete(new(entities.UserRole))
	return errors.WithStack(result.Error)
}

func (a *UserRole) DeleteByRoleID(ctx context.Context, roleID string) error {
	result := GetUserRoleDB(ctx, a.DB).Where("role_id=?", roleID).Delete(new(entities.UserRole))
	return errors.WithStack(result.Error)
}

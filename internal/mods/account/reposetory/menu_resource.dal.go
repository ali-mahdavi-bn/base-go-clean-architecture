...
func GetMenuResourceDB(ctx context.Context, defDB *gorm.DB) *gorm.DB {
	return util.GetDB(ctx, defDB).Model(new(entities.MenuResource))
}

// Menu resource management for AccountModules
type MenuResource struct {
	DB *gorm.DB
}

// Query menu resources from the database based on the provided parameters and options.
func (a *MenuResource) Query(ctx context.Context, params entities.MenuResourceQueryParam, opts ...entities.MenuResourceQueryOptions) (*entities.MenuResourceQueryResult, error) {
	var opt entities.MenuResourceQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}
	...
	var list entities.MenuResources
	pageResult, err := util.WrapPageQuery(ctx, db, params.PaginationParam, opt.QueryOptions, &list)
	if err != nil {
		return nil, errors.WithStack(err)
	}

	queryResult := &entities.MenuResourceQueryResult{
...
func (a *MenuResource) Get(ctx context.Context, id string, opts ...entities.MenuResourceQueryOptions) (*entities.MenuResource, error) {
	var opt entities.MenuResourceQueryOptions
	if len(opts) > 0 {
		opt = opts[0]
	}

	item := new(entities.MenuResource)
...
func (a *MenuResource) Create(ctx context.Context, item *entities.MenuResource) error {
	result := GetMenuResourceDB(ctx, a.DB).Create(item)
	return errors.WithStack(result.Error)
}

// Update the specified menu resource in the database.
func (a *MenuResource) Update(ctx context.Context, item *entities.MenuResource) error {
...
func (a *MenuResource) Delete(ctx context.Context, id string) error {
	result := GetMenuResourceDB(ctx, a.DB).Where("id=?", id).Delete(new(entities.MenuResource))
	return errors.WithStack(result.Error)
}

// Deletes the menu resource by menu id.
func (a *MenuResource) DeleteByMenuID(ctx context.Context, menuID string) error {
	result := GetMenuResourceDB(ctx, a.DB).Where("menu_id=?", menuID).Delete(new(entities.MenuResource))
	return errors.WithStack(result.Error)
}

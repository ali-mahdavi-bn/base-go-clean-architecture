...
// Login management for AccountModules
type Login struct {
	Cache       cachex.Cacher
	Auth        jwtx.Auther
	UserDAL     *reposetory.User
	UserRoleDAL *reposetory.UserRole
	MenuDAL     *reposetory.Menu
	UserBIZ     *User
}
...
func (a *Login) ParseUserID(c *gin.Context) (string, error) {
	...
	user, err := a.UserDAL.Get(ctx, userID, entities.UserQueryOptions{
		QueryOptions: util.QueryOptions{SelectFields: []string{"status"}},
	})
	if err != nil {
		return "", err
	} else if user == nil || user.Status != entities.UserStatusActivated {
		return "", invalidToken
	}
...
// This function generates a new captcha ID and returns it as a `entities.Captcha` struct. The length of
// the captcha is determined by the `config.C.Util.Captcha.Length` configuration value.
func (a *Login) GetCaptcha(ctx context.Context) (*entities.Captcha, error) {
	return &entities.Captcha{
...
func (a *Login) genUserToken(ctx context.Context, userID string) (*entities.LoginToken, error) {
...
func (a *Login) Login(ctx context.Context, formItem *entities.LoginForm) (*entities.LoginToken, error) {
	// verify captcha
	...
	// get user info
	user, err := a.UserDAL.GetByUsername(ctx, formItem.Username, entities.UserQueryOptions{
	...
	if err != nil {
		...
		return nil, errors.BadRequest(config.ErrInvalidUsernameOrPassword, "Incorrect username or password")
	} else if user.Status != entities.UserStatusActivated {
...
func (a *Login) RefreshToken(ctx context.Context) (*entities.LoginToken, error) {
...
// Get user info
func (a *Login) GetUserInfo(ctx context.Context) (*entities.User, error) {
	if util.FromIsRootUser(ctx) {
		return &entities.User{
			...
			Status:   entities.UserStatusActivated,
		}, nil
	}
	...
	userRoleResult, err := a.UserRoleDAL.Query(ctx, entities.UserRoleQueryParam{
		UserID: userID,
	}, entities.UserRoleQueryOptions{
		JoinRole: true,
	})
	if err != nil {
		return nil, err
...
func (a *Login) UpdatePassword(ctx context.Context, updateItem *entities.UpdateLoginPassword) error {
	if util.FromIsRootUser(ctx) {
...
func (a *Login) QueryMenus(ctx context.Context) (entities.Menus, error) {
	menuQueryParams := entities.MenuQueryParam{
	...
	menuResult, err := a.MenuDAL.Query(ctx, menuQueryParams, entities.MenuQueryOptions{
		QueryOptions: util.QueryOptions{
			OrderFields: entities.MenusOrderParams,
		},
	})
	if err != nil {
		return nil, err
	...
	if parentIDs := menuResult.Data.SplitParentIDs(); len(parentIDs) > 0 {
		...
		if len(missMenusIDs) > 0 {
			parentResult, err := a.MenuDAL.Query(ctx, entities.MenuQueryParam{
				InIDs: missMenusIDs,
			})
			if err != nil {
				return nil, err
...
func (a *Login) UpdateUser(ctx context.Context, updateItem *entities.UpdateCurrentUser) error {
	if util.FromIsRootUser(ctx) {

...
// Menu management for AccountModules
type Menu struct {
	Cache           cachex.Cacher
	Trans           *util.Trans
	MenuDAL         *reposetory.Menu
	MenuResourceDAL *reposetory.MenuResource
	RoleMenuDAL     *reposetory.RoleMenu
}
...
func (a *Menu) createInBatchByParent(ctx context.Context, items entities.Menus, parent *entities.Menu) error {
	total := len(items)
	for i, item := range items {
		...
		if !exist {
			...
			if item.Status == "" {
				item.Status = entities.MenuStatusEnabled
			}
			if item.Sequence == 0 {
...
func (a *Menu) Query(ctx context.Context, params entities.MenuQueryParam) (*entities.MenuQueryResult, error) {
	...
	result, err := a.MenuDAL.Query(ctx, params, entities.MenuQueryOptions{
	...
	if params.IncludeResources {
		for i, item := range result.Data {
			resResult, err := a.MenuResourceDAL.Query(ctx, entities.MenuResourceQueryParam{
				MenuID: item.ID,
			})
			if err != nil {
				return nil, err
...
// Query menus from the data access object based on the provided parameters and options.
func (a *Menu) PQuery(ctx context.Context, params entities.MenuQueryParam) (*entities.MenuQueryResult, error) {
	params.Pagination = false
...
func (a *Menu) fillQueryParam(ctx context.Context, params *entities.MenuQueryParam) error {
	if params.CodePath != "" {
		var (
			codes    []string
			lastMenu entities.Menu
		)
		for _, code := range strings.Split(params.CodePath, util.TreePathDelimiter) {
			...
			menu, err := a.MenuDAL.GetByCodeAndParentID(ctx, code, lastMenu.ParentID, entities.MenuQueryOptions{
				QueryOptions: util.QueryOptions{
...
func (a *Menu) appendChildren(ctx context.Context, data entities.Menus) (entities.Menus, error) {
	if len(data) == 0 {
		return data, nil
	}
	...
	for _, item := range data {
		childResult, err := a.MenuDAL.Query(ctx, entities.MenuQueryParam{
...
func (a *Menu) Get(ctx context.Context, id string) (*entities.Menu, error) {
	...
	menuResResult, err := a.MenuResourceDAL.Query(ctx, entities.MenuResourceQueryParam{
		MenuID: menu.ID,
	})
	if err != nil {
		return nil, err
...
func (a *Menu) Create(ctx context.Context, formItem *entities.MenuForm) (*entities.Menu, error) {
	menu := &entities.Menu{
		ID:        util.NewXID(),
		CreatedAt: time.Now(),
	}
...
func (a *Menu) Update(ctx context.Context, id string, formItem *entities.MenuForm) error {
	menu, err := a.MenuDAL.Get(ctx, id)
	if err != nil {
		return err
	} else if menu == nil {
